...
def _feature_paths_for_video(video_path: Path, config: dict[str, Any]) -> dict[str, str]:
    """Return target formal-modality source feature paths for a video.

    The paths are returned whether or not the files currently exist.  This lets
    the raw feature builder know where missing source features should be
    written later.
    """
...
def _feature_exists(feature_paths: dict[str, str]) -> dict[str, bool]:
    """Return existence flags for formal-modality source feature paths."""
    return {key: resolve_path(path).exists() for key, path in feature_paths.items()}
            ...
            samples.append(
                {
                    ...
                    "feature_exists": _feature_exists(feature_paths),
