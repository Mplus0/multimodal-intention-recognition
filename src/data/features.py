...
project-relative configuration. Cache files may speed repeated checks. When a
cached sample and source ``.npy`` files are both absent, the Member A raw
feature builder can generate clean source features from ``sample["raw_paths"]``.
"""
...
from src.data.raw_feature_builder import RawFeatureBuildError, ensure_sample_source_features
...
def load_or_build_sample_features(
    ...
    if missing_sources:
        if sample.get("raw_paths"):
            try:
                built_paths = ensure_sample_source_features(sample, config)
            except RawFeatureBuildError as error:
                joined = "; ".join(missing_sources)
                raise FileNotFoundError(
                    "Cannot build five-modality features because source feature files are missing "
                    "and raw feature construction failed. "
                    f"Missing sources before raw build: {joined}. "
                    f"Raw build error: {error}"
                ) from error

            source_paths = {}
            still_missing = []
            for key in MODALITY_KEYS:
                path = built_paths.get(key)
                if path is None or not path.exists():
                    still_missing.append(f"{key}: {path if path is not None else 'not returned'}")
                else:
                    source_paths[key] = path
            if still_missing:
                joined = "; ".join(still_missing)
                raise FileNotFoundError(
                    "Raw feature builder returned but source feature files are still missing. "
            ...
            raise FileNotFoundError(
                "Cannot build five-modality features because source feature files are missing "
                "and sample metadata does not provide raw_paths for automatic construction. "
