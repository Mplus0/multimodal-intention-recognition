...
from src.data.raw_feature_builder import ensure_source_features
...
def load_or_build_sample_features(
    ...
    Complete-sample ``.npz`` cache is used first. If source ``.npy`` files are
    missing and raw building is enabled, the missing source features are built
    from ``sample["raw_paths"]`` and then cached for future runs. Missing
    audio/text dependencies raise explicit errors instead of fabricated arrays.
    """
    if config is None:
        config = load_config()
    ...
    def collect_source_paths() -> tuple[dict[str, Path], list[str]]:
        paths: dict[str, Path] = {}
        missing: list[str] = []
        for modality in MODALITY_KEYS:
            path = _metadata_path(sample, modality)
            if path is None or not path.exists():
                missing.append(f"{modality}: {path if path is not None else 'not provided'}")
            else:
                paths[modality] = path
        return paths, missing

    source_paths, missing_sources = collect_source_paths()
    if missing_sources and build_missing_source_features:
        generated_paths = ensure_source_features(sample, config, rebuild=rebuild_source_features)
        sample["feature_paths"] = {key: str(path) for key, path in generated_paths.items()}
        source_paths, missing_sources = collect_source_paths()
    ...
    if missing_sources:
        ...
        raise FileNotFoundError(
            ...
            "Run with build_missing_source_features=True or prebuild source features from raw data. "
